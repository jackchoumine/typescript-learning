...
type 和 interface 之间的区别很模糊。

只需要了解它们的区别，哪种情况使用哪种，且保持一致，更进一步，知道如何使用两种方式表示同一个类型。

相同点：

- 都可声明函数类型，当函数有属性时，接口声明更加符合直觉：函数是可调用的对象。
- 都可表示元组和数组，但是接口繁琐。
- 都可接收泛型参数。
- 都可扩展：接口使用 extends，别名使用 `&`。
...
1. 对象一般使用接口，数组、函数、元组等使用别名。

2. 允许使用者扩增，使用接口，否则使用别名。

3. 复杂类型(比如 typeof)只能使用别名。
